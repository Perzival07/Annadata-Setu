...
async def resolve_marathi_script(diagnosis, passport=None) -> str:
    """Spoken Marathi, generated by brain where possible.

    BRAIN.md §11 (15:30): ask for Marathi directly rather than translating. The
    local template is the fallback, and is Marathi-only by construction — the
    voice note is synthesised with an mr-IN voice, so a part-English script comes
    out mispronounced.
    """
    try:
        script = await compose_marathi_script(diagnosis, passport)
        if script:
            return script
    except Exception as e:
